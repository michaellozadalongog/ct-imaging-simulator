...
class MedTechSuiteApp:
    def __init__(self, root):
        ...
        # PANEL 5: HIPAA CYBER-SECURITY STRIPPER
        p5_frame = ttk.LabelFrame(main_frame, text=" Project 5: Automated DICOM HIPAA Data Anonymizer ", padding="12")
        p5_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p5_frame, text="Scans directory paths for raw data payloads, intercepts rigid hexadecimal storage tags, strips personal metrics, and commits cryptographic hashed keys to isolated outputs.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        ttk.Button(p5_frame, text="Batch Sanitize Local DICOM Headers", command=self.run_anonymizer, style="Action.TButton").pack(anchor=tk.W)
        
        # PANEL 6: MILITARY EQUIPMENT CMMS DATABASE
        p6_frame = ttk.LabelFrame(main_frame, text=" Project 6: Relational Maintenance CMMS Database Logs ", padding="12")
        p6_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p6_frame, text="Queries SQLite database backend tables tracking asset compliance profiles. Flags expiring high-risk assets and generates open corrective schedule backlog text modules.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        btn_layout = ttk.Frame(p6_frame)
    ...
    def view_database_logs(self):
        ...
        for o in orders:
            txt.insert(tk.END, f"📋 Order #{o[0]} | Asset ID: {o[1]} | Type: {o[2]} | Status: {o[3]}\n")
