...
class MedTechSuiteApp:
    def __init__(self, root):
        ...
        self.root.geometry("700x680")  # Expanded height to cleanly hold all 4 panels
        ...
        # --------------------------------------------------------
        # NEW PANEL 2: PROJECT 2 - X-RAY THERMAL SIMULATOR
        # --------------------------------------------------------
        p2_frame = ttk.LabelFrame(main_frame, text=" Project 2: X-Ray Tube Thermodynamic Simulator ", padding="15")
        p2_frame.pack(fill=tk.X, pady=10)
        p2_desc = ttk.Label(p2_frame, text="Models real-time X-ray tube heat accumulation and cooling using Newton's Law of Cooling. Simulates automated safety lockouts and plots continuous dual-compartment dissipation curves.", wrap=600)
        p2_desc.pack(anchor=tk.W, pady=(0, 10))
        p2_btn = ttk.Button(p2_frame, text="Launch Thermal Physics Engine & Input Parameters", command=self.run_thermal_simulator, style="Action.TButton")
        p2_btn.pack(anchor=tk.W)
        
        # --------------------------------------------------------
        # PANEL 3: PROJECT 3 - PACS & DICOM NETWORKING NODE
        # --------------------------------------------------------
        p3_frame = ttk.LabelFrame(main_frame, text=" Project 3: DICOM PACS Server Network Pipeline ", padding="15")
        p3_frame.pack(fill=tk.X, pady=10)
        p3_desc = ttk.Label(p3_frame, text="Spuns up an internal DICOM listening node archive on port 4242 to intercept data traffic, generates a mock structural .dcm study, and triggers a C-STORE data network transmission loopback.", wrap=600)
        p3_desc.pack(anchor=tk.W, pady=(0, 10))
        p3_btn = ttk.Button(p3_frame, text="Launch Port 4242 Server & Transmit Scan Data", command=self.run_dicom_pipeline, style="Action.TButton")
        p3_btn.pack(anchor=tk.W)
        
        # --------------------------------------------------------
        # PANEL 4: PROJECT 4 - CLINICAL BIOMED ASSET CMMS
        # --------------------------------------------------------
        p4_frame = ttk.LabelFrame(main_frame, text=" Project 4: Relational Maintenance CMMS Database Logs ", padding="15")
        p4_frame.pack(fill=tk.X, pady=10)
        p4_desc = ttk.Label(p4_frame, text="Queries the SQLite relational backend tables tracking military hospital clinical assets. Automatically flags expiring high-risk inventory items and prints active open work schedules.", wrap=600)
        p4_desc.pack(anchor=tk.W, pady=(0, 10))
        
        btn_layout = ttk.Frame(p4_frame)
        btn_layout.pack(anchor=tk.W)
        p4_btn = ttk.Button(btn_layout, text="Calculate Compliance & Open Work Orders", command=self.run_cmms_automation, style="Action.TButton")
        p4_btn.grid(row=0, column=0, padx=(0, 10))
        p4_view_btn = ttk.Button(btn_layout, text="View Database Schedule Logs", command=self.view_database_logs, style="Action.TButton")
        p4_view_btn.grid(row=0, column=1)
    ...
    def run_thermal_simulator(self):
        try:
            messagebox.showinfo("Processing", "Launching thermodynamic simulation console. Look at your Terminal window to enter exposure metrics!")
            # Open script in a way that allows users to interact with terminal text inputs
            os.system("python3 xray_thermal_sim.py &")
        except Exception as e:
            messagebox.showerror("System Error", f"Could not launch thermal engine: {str(e)}")
