...
class MedTechSuiteApp:
    def __init__(self, root):
        ...
        # NEW PANEL 5: INFUSION PUMP FLUID MECHANICS
        ...
        # PANEL 6: HIPAA CYBER-SECURITY STRIPPER
        p6_frame = ttk.LabelFrame(main_frame, text=" Project 6: Automated DICOM HIPAA Data Anonymizer ", padding="12")
        p6_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p6_frame, text="Scans directory paths for raw data payloads, intercepts rigid hexadecimal storage tags, strips personal metrics, and commits cryptographic hashed keys to isolated outputs.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        ttk.Button(p6_frame, text="Batch Sanitize Local DICOM Headers", command=self.run_anonymizer, style="Action.TButton").pack(anchor=tk.W)
        
        # PANEL 7: MILITARY EQUIPMENT CMMS DATABASE
        p7_frame = ttk.LabelFrame(main_frame, text=" Project 7: Relational Maintenance CMMS Database Logs ", padding="12")
        p7_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p7_frame, text="Queries SQLite database backend tables tracking asset compliance profiles. Flags expiring high-risk assets and generates open corrective schedule backlog text modules.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        btn_layout = ttk.Frame(p7_frame)
