...
class MedTechSuiteApp:
    def __init__(self, root):
        ...
        # 🟢 ADDED PANEL 6: ULTRASOUND PIEZOELECTRIC FAULT ANALYZER
        ...
        # PANEL 7: HIPAA CYBER-SECURITY STRIPPER
        p7_frame = ttk.LabelFrame(main_frame, text=" Project 7: Automated DICOM HIPAA Data Anonymizer ", padding="12")
        p7_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p7_frame, text="Scans directory paths for raw data payloads, intercepts rigid hexadecimal storage tags, strips personal metrics, and commits cryptographic hashed keys to isolated outputs.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        ttk.Button(p7_frame, text="Batch Sanitize Local DICOM Headers", command=self.run_anonymizer, style="Action.TButton").pack(anchor=tk.W)
        
        # PANEL 8: MILITARY EQUIPMENT CMMS DATABASE
        p8_frame = ttk.LabelFrame(main_frame, text=" Project 8: Relational Maintenance CMMS Database Logs ", padding="12")
        p8_frame.pack(fill=tk.X, pady=8)
        ttk.Label(p8_frame, text="Queries SQLite database backend tables tracking asset compliance profiles. Flags expiring high-risk assets and generates open corrective schedule backlog text modules.", wrap=650).pack(anchor=tk.W, pady=(0, 5))
        btn_layout = ttk.Frame(p8_frame)
    ...
    def view_database_logs(self):
        ...
        for o in orders:
            txt.insert(tk.END, f"📋 Order #{o} | Asset ID: {o} | Type: {o} | Status: {o}\n")
